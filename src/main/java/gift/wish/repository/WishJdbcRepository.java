...
public class WishJdbcRepository implements WishRepository{
    ...
    private static final RowMapper<Wish> wishRowMapper = (rs, rowNum) -> {
        Member member = new Member(rs.getLong("member_id"), null, null, null);
        Product product = new Product(rs.getLong("product_id"), null, 0, null);
        return new Wish(member, product, rs.getInt("quantity"));
    };
